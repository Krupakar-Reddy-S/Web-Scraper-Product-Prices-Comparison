...
def fetch_amazon(search_query, results):
    try:
        ...
        print("Connection error occurred while fetching data from Amazon. Skipping this source.")
...
def fetch_snapdeal(search_query, results):
    try:
        ...
        print("Connection error occurred while fetching data from Snapdeal. Skipping this source.")
...
def fetch_flipkart1(search_query, results):
    try:
        ...
        print("Connection error occurred while fetching data from Flipkart1. Skipping this source.")
...
def fetch_flipkart2(search_query, results):
    try:
        ...
        print("Connection error occurred while fetching data from Flipkart2. Skipping this source.")
...
def fetch_flipkart3(search_query, results):
    try:
        ...
        print("Connection error occurred while fetching data from Flipkart3. Skipping this source.")
...
def products_validation(products):
    ...
    print(f"Validation Limit: ₹{int(average_product_price * 0.75)} to ₹{int(average_product_price * 2.00)}")

    products_copy = products.copy()

    for serial, product in products_copy.items():
        if product.get('product_price', 0) < (average_product_price * 0.50) or product.get('product_price', 0) > (average_product_price * 2.00):
            del products[serial]
...
def plot_prices_with_links_and_tooltips(product_results, search_query):
    ...
    ax.plot(x, product_prices, marker='o', linestyle='-', markersize=8, lw=2, alpha=0.9)
    tooltips = []

    buffer_x = 0.25

    y_range = max(product_prices) - min(product_prices)
    ...
    ax.set_title(f"Product Prices for: {search_query}")
...
if not results.empty():
    ...
    print("\nNo results were fetched. Check your network connection or search query.")
